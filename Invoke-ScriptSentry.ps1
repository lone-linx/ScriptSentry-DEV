...
function Get-LogonScripts {
    ...
    Write-Host "[*] Getting Logon Scripts" -ForegroundColor Cyan

    # Get the current domain name from the environment
    # $currentDomain = [System.DirectoryServices.ActiveDirectory.Domain]::GetCurrentDomain()
    if ($Domain) {
        Write-Host "`t[DEBUG] Domain: $Domain"
        $Domains = $Domain
    } else {
        $Domains = Get-ForestDomains
        Write-Host "`t[DEBUG] Got Forest Domains: $($Domains)"
    }

    foreach ($D in $Domains) {
        # $SysvolScripts = '\\' + (Get-ADDomain).DNSRoot + '\sysvol\' + (Get-ADDomain).DNSRoot + '\scripts'
        if ($Server) {
            $SysvolScripts = "\\$($Server)\sysvol\$($Domain)\scripts" 
        } else {
            $SysvolScripts = "\\$($D.Name)\sysvol\$($D.Name)\scripts"
        }

        Write-Host "`t[DEBUG] Getting SYSVOL at: $SysvolScripts"
...
function Get-GPOLogonScripts {
    ...
    Write-Host "[*] Getting GPO Logon Scripts" -ForegroundColor Cyan
    # Get the current domain name from the environment
    # $currentDomain = [System.DirectoryServices.ActiveDirectory.Domain]::GetCurrentDomain()
    if ($Domain) {
        Write-Host "`t[DEBUG] Domain: $Domain"
    ...
    foreach ($Domain in $Domains) {
        if ($Server) {
            $PoliciesPath = "\\$Server\SYSVOL\$($Domain)\POLICIES\"
        } else {
            $PoliciesPath = "\\$($Domain.Name)\SysVol\$($Domain)\Policies"
        }
        Write-Host "`t[DEBUG] Getting policies at $PoliciesPath"

        $Policies = Get-ChildItem $PoliciesPath -ErrorAction SilentlyContinue
...
function Find-AdminLogonScripts {
    ...
    Foreach ($Admin in $AdminUsers) {
        if ($Credential.UserName) {
            $AdminLogonScripts = Get-DomainUser -Identity $Admin.MemberName -Credential $Credential | Where-Object { $null -ne $_.scriptPath }
        } else {
...
function Find-LogonScriptCredentials {
    ...
    foreach ($script in $LogonScripts) {
        ...
        $Credentials = Get-Content -Path $script.FullName -ErrorAction SilentlyContinue | Select-String -Pattern "/u[\w]*:","-AsPlainText","password\s*=","secret\s*=" -AllMatches
        if ($Credentials) {
            # "`n[!] CREDENTIALS FOUND!"
...
function Find-UNCScripts {
    ...
    Write-Host '[*] Finding UNC Scripts' -ForegroundColor Cyan
...
function Find-MappedDrives {
    ...
    Write-Host "[*] Finding Mapped Drives" -ForegroundColor Cyan
    $Shares = @()
...
function Find-NonexistentShares {
    ...
    Write-Host "[*] Finding Nonexistent Shares" -ForegroundColor Cyan
...
function Find-UnsafeLogonScriptPermissions {
    ...
    Write-Host "[*] Finding Unsafe Logon Script Permissions" -ForegroundColor Cyan
...
function Find-UnsafeUNCPermissions {
    ...
    Write-Host "[*] Finding Unsafe UNC Permissions" -ForegroundColor Cyan
...
function Find-UnsafeLogonScriptPermissions {
    ...
    Write-Host "[*] Finding Unsafe LogonScriptPermissions" -ForegroundColor Cyan
...
function Find-UnsafeGPOLogonScriptPermissions {
    ...
    Write-Host "[*] Finding Unsafe GPO Logon Script Permissions" -ForegroundColor Cyan
...
if ($Credential.UserName) {
    ...
    Write-Host "[i] No credentials supplied. Running ScriptSentry as is."
}
...
if ($Credential.UserName) {
    ...
    #Write-Host $GPOLogonScripts

    if ($LogonScripts) {
        Write-Host "[+] Logon scripts found!`n" -ForegroundColor Cyan
        # Write-Host $LogonScripts | fl
        # foreach ($script in $LogonScripts) {
    ...
    $NetlogonSysvol = Get-NetlogonSysvol -Server $Server $Domain
	if ($NetlogonSysvol) {
    ...
    Write-Host "[i] No domain credentials supplied. Running ScriptSentry as SYSTEM." -ForegroundColor Cyan
    ...
    if ($LogonScripts) {
        ...
        Write-Host "[i] No logon scripts found!" -ForegroundColor Cyan
    }
    ...
    if ($UNCScripts) {
        ...
        Write-Host "[i] No UNC files found!" -ForegroundColor Cyan
    }
    ...
    if ($MappedDrives) {
        ...
        Write-Host "[i] No mapped drives found!" -ForegroundColor Cyan
    }
    ...
    if ($GPOLogonScripts) {
        ...
        Write-Host "[i] No GPO logon scripts found!" -ForegroundColor Cyan
    }
...
if ($NonExistentShares) {
    ...
    Write-Host "[i] No non-existent shares found!" -ForegroundColor Cyan
}
