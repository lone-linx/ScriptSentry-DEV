...
function Get-LogonScripts {
    ...
    if ($Domain) {
        Write-Host "[DEBUG] Domain: $Domain"
        $Domains = $Domain
    } else {
    ...
    foreach ($Domain in $Domains) {
        ...
        if ($Server) {
            Write-Host "[DEBUG] Getting SYSVOL at: \\$Server\sysvol\$($Domain)\scripts"
...
function Get-GPOLogonScripts {
    ...
    Write-Host "[DEBUG] Getting GPO Logon Scripts at Server: $Server and Domain: $Domain"
    # Get the current domain name from the environment
    # $currentDomain = [System.DirectoryServices.ActiveDirectory.Domain]::GetCurrentDomain()
    if ($Domain) {
        $Domains = $Domain
    ...
    foreach ($Domain in $Domains) {
        if ($Server) {
            Write-Host "[DEBUG] Getting policies at \\$Server\SYSVOL\$($Domain)\POLICIES\"
            $Policies = Get-ChildItem "\\$Server\SYSVOL\$($Domain)\POLICIES\" -ErrorAction SilentlyContinue
        } else {
            $Policies = Get-ChildItem "\\$($Domain.Name)\SysVol\$($Domain)\Policies" -ErrorAction SilentlyContinue
        }
...
function Find-UNCScripts {
    ...
    Write-Host '[DEBUG] Finding UNC Scripts'
...
function Find-MappedDrives {
    ...
    Write-Host "[DEBUG] Finding Mapped Drives"
    $Shares = @()
...
function Find-NonexistentShares {
    ...
    Write-Host "[DEBUG] Finding Nonexistent Shares"
...
function Find-UnsafeLogonScriptPermissions {
    ...
    Write-Host "[DEBUG] Finding Unsafe Logon Script Permissions"
...
function Find-UnsafeUNCPermissions {
    ...
    Write-Host "[DEBUG] Finding Unsafe UNC Permissions"
...
function Find-UnsafeGPOLogonScriptPermissions {
    ...
    Write-Host "[DEBUG] Finding Unsafe GPO Logon Script Permissions"
...
if ($Credential.UserName) {
    ...
    if ($socks) {
        Write-Host "Getting logon scripts from Sever: $Server and Domain: $Domain"
